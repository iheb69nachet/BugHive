...
class BugHive
{
    ...
    public function handle(Throwable $exception, $fileType = 'php', array $customData = [])
    {
        ...
        if ($fileType == 'javascript') {
            ...
            $count = config('bughive.lines_count');
    ...
    {
        if (count(config('bughive.environments')) == 0) {
            return true;
        }

        if (in_array(App::environment(), config('bughive.environments'))) {
            return false;
        }
    ...
    /**
     * Get the last exception id given to us by the bughive API.
     * @return string|null
     */
    ...
    public function getExceptionData(Throwable $exception)
    {
        ...
        $data['release'] = config('bughive.release', null);
        $data['storage'] = [
            'SERVER' => [
        ...
        // Get project version
        $data['project_version'] = config('bughive.project_version', null);
    ...
    {
        return in_array($exceptionClass, config('bughive.except'));
    }
    ...
    {
        return 'bughive.' . Str::slug($data['host'] . '_' . $data['method'] . '_' . $data['exception'] . '_' . $data['line'] . '_' . $data['file'] . '_' . $data['class']);
    }
    ...
    public function getUser()
    {
        if (function_exists('auth') && (app() instanceof \Illuminate\Foundation\Application && auth()->check())) {
            ...
            if ($user instanceof \bughive\Concerns\BugHivable) {
    ...
    public function addExceptionToSleep(array $data)
    {
        ...
        return Cache::put($exceptionString, $exceptionString, config('bughive.sleep'));
    }
}